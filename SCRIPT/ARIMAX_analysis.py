...
for i, country in enumerate(COUNTRIES):
    ...
    # Fit ARIMAX
    model = SARIMAX(co2_train, exog=gdp_train_scaled, order=order, enforce_stationarity=False, enforce_invertibility=False)
    ...
    ax.plot(co2_test.index, forecast_mean, color="red", linestyle="--", label="ARIMAX forecast", linewidth=2)
    ax.fill_between(co2_test.index, forecast_ci.iloc[:, 0], forecast_ci.iloc[:, 1], color="red", alpha=0.15, label="95% CI")
...
bars1 = ax2.bar(x - width / 2, results_df["SARIMA_RMSE"], width, label="SARIMA (CO2 only)", color="steelblue", alpha=0.8)
bars2 = ax2.bar(x + width / 2, results_df["ARIMAX_RMSE"], width, label="ARIMAX (CO2 + GDP)", color="coral", alpha=0.8)
...
display_cols = ["Country", "order", "SARIMA_RMSE", "ARIMAX_RMSE","RMSE_change_pct", "SARIMA_AIC", "ARIMAX_AIC",
                "GDP_coef", "GDP_pval"]
